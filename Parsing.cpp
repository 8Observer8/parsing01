...
bool CParsing::parseData(std::string buffer, bool& dataIsReady) {
    ...
    if (m_storageBufferArr.size() > 100) {
    ...
    if (m_storageBufferArr.empty()) {
        ...
        if (beginOfPackage != "$GP") {
            if (buffer.find("$GP")) {
                buffer = getBufferStartWithPattern(buffer, "$GP");
            } else {
                dataIsReady = false;
                m_GGABuffer.clear();
                m_RMCBuffer.clear();
        ...
        storage(m_storageBufferArr, bufferArr);
    }
...
std::string CParsing::getBufferStartWithPattern(std::string buffer,
        std::string pattern) {
    std::string result;
    std::size_t found = buffer.find(pattern);
    if (found != std::string::npos) {
        result = buffer.substr(found);
    } else {
        result = "Not found";
    }
    return result;
...
bool CParsing::haveWeAllPackages() {
    ...
    int nsize;
    if (m_storageBufferArr.back() == "") {
        nsize = m_storageBufferArr.size() - 1;
    } else {
        nsize = m_storageBufferArr.size();
    }
